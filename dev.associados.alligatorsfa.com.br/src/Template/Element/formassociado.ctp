<div class="associados form large-9 medium-8 columns content">
    ...
    <fieldset>
        ...
        <?php
            ...
            echo $this->Form->input('foto',['type'=>(isset($escondefoto)?$escondefoto:'file'),'name'=>'foto','label'=>'Foto: Tamanho max permitido (640 Kbytes)']);
	?>
	...
	<script>
	  function handleFileSelect(evt) {
	    ...
	    for (var i = 0, f; f = files[i]; i++) {
              cor = (f.size > 640000) ? 'red' : 'green';
	      kb = Math.floor(f.size/1000);
	      output.push('<li><strong>', escape(f.name), '</strong> (', f.type || 'n/a', ') - tamanho: <font color="',cor,'">',
			  kb, ' Kbytes</font>',
			  '</li>');
	    }
